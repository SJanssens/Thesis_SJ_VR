function [path, j1, j2] = hmmViterbiC_endnodes(logpi, logA, logB,endnodes)
...
%%
if nargin<4
    % specify all nodes as endnodes
    endnodes = ones(size(logpi));
end
...
[p, path(T)] = max(delta(:,T).*endnodes); % only endnodes can have non-zero probs
for t=T-1:-1:1
