function [x,esq,j] = kmeans_correct(d,x0,maxiter,computetype)
...
BlkSz=1e5;
if nargin <4
    computetype=2; % use mex by default
end
...
while any(x(:) ~= y(:))
   ...
   kk=0;
   while kk<n,
       ssel=kk+1:min(kk+BlkSz,n);
       
       z = eucdist(x,d(:,ssel),computetype); 
       
       [m(ssel),j(ssel)] = min(z,[],2); % for every data point, the best prototype
       kk=kk+BlkSz;
   end
   y = x;
   ToBeRemoved=logical(zeros(1,k));
   for i=1:k
      s = j==i; % sselect the data points for which i is the best prototype
      if any(s)
         x(:,i) = mean(d(:,s),2);
      else
         ToBeRemoved(i)=1;
...
m=zeros(n,1);j=m;
kk=0;
while kk<n, %once more to determine membership corresponding to the last mean estimation of the clusters
     ssel=kk+1:min(kk+BlkSz,n);
