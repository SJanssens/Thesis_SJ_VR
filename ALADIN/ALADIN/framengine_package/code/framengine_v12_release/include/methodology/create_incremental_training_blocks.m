...
function [train_utterance_block,test_utterance_block,train_blocks,test_blocks]=create_incremental_training_blocks(seed, blocks, blockstep_array, n_experiment)
%input
...
[ A ] = Latin_square(n_blocks, seed );%A is a latin square matrix n_blocks X n_blocks
...
for i=1:n_experiment
    for j=1:length(blockstep_array)
        train_blocks{i,j}=A(i,1:blockstep_array(j)); % if blockstep_array=[1 3 4 10], then 1, 3,4, 10 first entries in the A-rows are incorporated in the training sets
        test_blocks{i,j}=A(i,blockstep_array(j)+1:end);% remainder for test
